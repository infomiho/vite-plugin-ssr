...
If you use <Link text="Client Routing" href="/client-routing" />, then *all* your Route Functions are loaded in the browser. This means that if a Route Function imports a lot of code, then all that code is loaded on the client-side and on every pages. A heavy Route Function slows down the loading of each page.

You should keep Route Functions both lightweight and fast.
...
The motivation for an async Route Function is often redirection and/or protecting a private page; instead use <Link href="/render" text={<code>throw render()</code>} /> or <Link href="/redirect" text={<code>throw redirect()</code>} /> with an async <Link text={<code>guard()</code>} href="/guard" /> or <Link text={<code>onBeforeRender()</code>} href="/onBeforeRender" /> hook.

> An asynchronous Route Function would slow down your entire app: as we have seen in [Lightweight & fast](#lightweight-fast), every time the user navigates to a new page *all* Route Functions are called. This means that a slow Route Function slows down *all* pages.
...
That said, you can workaround it by misusing `pageContext.routeParams` to provide data.
