...
When implementing SSR, the cient-side `render()` hook works in tandem with the <Link text={<>server-side <code>render()</code> hook</>} href="/render-hook" />: the server-side `render()` hook renders the page to HTML and the client-side `render()` hook <Link text="hydrates" href="/hydration" /> the HTML.
