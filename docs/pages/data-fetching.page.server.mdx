...
For handling errors, we can use <Link href="/render" text={<code>throw render()</code>}></Link> and <Link href="/redirect" text={<code>throw redirect()</code>}></Link>.
...
async function onBeforeRender(pageContext) {
  // The file path `/pages/movie/@id.page.server.js` contains `@id` which denotes a route parameter.
  ...
  if (response.status === 404) {
    ...
    redirect(302, '/movie/add')
    */
    /* Or render the movie submition form while preserving the URL:
    redirect(302, '/movie/add')
    */
  }
  // ...
...
Using `throw render('/movie/add')` instead of `throw redirect(302, '/movie/add')` is a novel technique we recommend an explain at <Link href="/auth#login-flow" />.
